...
export const portfolioAPI = {
  ...
  addPortfolio: async (newProduct) => {
    const res = await api.post('/portfolio', newProduct);
    return res.data;
  },

  // 포트폴리오 수정
  updatePortfolio: async (portfolioId, updateData) => {
    const res = await api.patch(`/portfolio/${portfolioId}`, updateData);
    return res.data;
  },
  ...
  getProductInfo: async (formData) => {
    const response = await api.post('/chat/product-info', {
